# copied from https://install.python-poetry.org/ 2025-10-01 with the following
# modifications made to make installation even easier for users of our demo package
# 1. we set POETRY_HOME env variable to demo folder
# 2. we modify the printed output to inform that it is a modified installer and update poetry.exe path
# 3. we name binary poetry.exe regardless of platform
# 4. we move poetry.exe from [demo_folder]\bin to just [demo_folder] and then we delete the bin-folder
# 5. we also remove VERSION file created by the poetry installer to have no side effects
...
# 1. we set POETRY_HOME env variable to demo folder
SCRIPT_FOLDER = Path.resolve(Path(__file__)).parent
DEMO_FOLDER = SCRIPT_FOLDER.parent
os.environ["POETRY_HOME"] = str(DEMO_FOLDER)
...
PRE_MESSAGE = """This will download and install the latest version of {poetry},
a dependency and package manager for Python.

It will add `poetry.exe` to {DEMO_FOLDER}.

To uninstall, just delete the whole demo folder.
"""
...
if __name__ == "__main__":
    try:
        ...
        for fn in ["poetry.exe", "poetry"]:
            with contextlib.suppress(Exception):
                shutil.copy(DEMO_FOLDER / "bin" / fn, DEMO_FOLDER / "poetry.exe")
        with contextlib.suppress(Exception):
            shutil.rmtree(DEMO_FOLDER / "bin")
        with contextlib.suppress(Exception):
            Path.unlink(DEMO_FOLDER / "VERSION", missing_ok=True)
